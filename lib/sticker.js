...
import { ImageProcessor } from "../Nova/tools/imgonline.js";
import { Uploader } from "../Nova/tools/uploader.js";
...
async function sticker3(img, url, packname, author) {
  try {
    ...
    let mime =
      typeof image.getMIME === "function" ? image.getMIME() : Jimp.MIME_WEBP;
    image.exifData = {
      artist: author,
      software: "Jimp",
      comment: packname
...
async function sticker7(input) {
  try {
    ...
    const result = upload.uploadPomf2
      ? await upload.uploadPomf2(buffer)
      : upload.hostfile
      ? await upload.hostfile(buffer)
      : upload.catbox
      ? await upload.catbox(buffer)
      : upload.mediaUpload
      ? await upload.mediaUpload(buffer)
      : (() => {
