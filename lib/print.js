...
class ClearLogger {
  ...
  clearLogs = () => (
    console.log(chalk.blue("Clearing log...")) && console.clear(),
    (this.logCount = 0)
  );
}
...
export default async function (
  ...
  if (m?.sender && m?.msg) {
    if (m?.msg && m?.isCommand) {
      ...
      console.log(
        `📨 ${chalk.bold.redBright("Message Info")}: ${chalk.yellow(
          "["
        )} ${chalk.green(m?.isGroup ? "Group" : "Private")} ${chalk.yellow(
          "]"
        )}`
      );
      console.log(
      ...
      console.log(
        `   ${chalk.bold.cyan("- Message Type")}: ${
          formatType(m.mtype) || "N/A"
        }`
      );
      console.log(
        `   ${chalk.bold.cyan("- Message ID")}: ${
          m.msg?.id || m.key.id || "N/A"
        }`
      );
      console.log(
        `   ${chalk.bold.cyan("- Sent Time")}: ${
          formatTime(m.messageTimestamp) || "N/A"
        }`
      );
      console.log(
        `   ${chalk.bold.cyan("- Message Size")}: ${
          formatSize(filesize || 0) || "N/A"
        }`
      );
      console.log(
        `   ${chalk.bold.cyan("- Sender ID")}: ${
          m.sender.split("@")[0] || m.key.remoteJid || "N/A"
        }`
      );
      console.log(
        `   ${chalk.bold.cyan("- Sender Name")}: ${
          m.name || m.pushName || conn.user.name || "N/A"
        }`
      );
      console.log(
        `   ${chalk.bold.cyan("- Chat ID")}: ${
          m.chat.split("@")[0] || m.key.remoteJid || "N/A"
        }`
      );
      console.log(`   ${chalk.bold.cyan("- Chat Name")}: ${chat || "N/A"}`);
      console.log(
        `   ${chalk.bold.cyan("- Total Log Messages")}: ${
          clearLoggerInstance.logCount
        }`
      );
      console.log(
        chalk.bold.cyan("╰──────────────────────────────────────···\n")
    ...
    if (
      (opts["antibot"] ||
        (global.db.data.settings[conn.user.jid].antibot && m?.isGroup)) &&
      m?.sender
    ) {
      clearLoggerInstance.addLog();
      ...
      if (
        ["BAE5", "WA"].some(
          (k) => idBot.includes(k) && m.sender !== conn.user.jid
        )
      ) {
        const antiBotMessage =
          "[ *🚫 ANTI BOT 🚫* ]\n\n🛑 Group ini dilengkapi dengan anti bot\n\n⚠ Anda melanggar peraturan bot";

        const thumbnail = await conn.getFile(
          "https://cdn-icons-png.flaticon.com/128/2333/2333083.png"
        );
    ...
    if (m?.isGroup && m?.sender) {
      ...
      if (
        ...
        ["BAE5", "WA"].some((k) => idBot.includes(k))
      ) {
        ...
        if (
          !conn.user.listbot[chatKey].some((bot) => bot.number === m.sender)
        ) {
          conn.user.listbot[chatKey].push(chatValue);
          conn.logger.info("New bot number " + m.sender.split("@")[0]);
        } else {
          conn.user.listbot[chatKey] = conn.user.listbot[chatKey].filter(
            (bot) => bot.number !== m.sender
          );
    ...
    if (m.mentionedJid) {
      for (const user of m.mentionedJid) {
        logMessage = logMessage.replace(
          "@" + user.split`@`[0],
          chalk.bold.blueBright("@" + (await conn.getName(user)))
        );
      }
    }
  ...
  if (m?.msg && m?.sender) {
    ...
    if (/document/i.test(attachmentType)) {
      ...
      console.log(`   ${chalk.bold.cyan("- Chat Name")}: ${chat || "N/A"}`);
      console.log(
        chalk.bold.redBright(`📄 Attached Document:
 ...
 ${m.msg.fileName || m.msg.displayName || "Document"}`)
      ...
      console.log(
        chalk.bold.redBright(
          `👨 Attached Contact: ${m.msg.displayName || "N/A"}`
        )
      );
      console.log(
        chalk.bold.cyan("╰──────────────────────────────────────···")
      ...
      console.log(`   ${chalk.bold.cyan("- Chat Name")}: ${chat || "N/A"}`);
      console.log(
        chalk.bold.redBright(`🎴 Attached Sticker: ${attachmentName}`)
      );
      console.log(
        chalk.bold.cyan("╰──────────────────────────────────────···")
      );
  ...
  if (m?.isCommand && m?.sender && m?.msg) {
    ...
    console.log(
      chalk.bold.blueBright(
        `  ${chalk.bold.red("To")}: ${getPhoneNumber(conn.user?.jid)}`
      )
    );
