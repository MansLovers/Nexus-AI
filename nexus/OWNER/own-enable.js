let nexus = async (m, { conn, command, args, isOwner, isAdmin, isROwner }) => {
  ...
  switch (type) {
    ...
    default:
      if (!/[01]/.test(command))
        return m.reply(
          `
*Berikut Ini Adalah Beberapa Option Enable/Disable*
...
`.trim()
  ...
  m.reply(`[✓] Permintaan *${isEnable ? "ON" : "OFF"}* ${type} Berhasil`);
};
nexus.help = ["enable", "disable"].map((v) => v + " [ OPTION ]");
nexus.tags = ["owner"];
nexus.command = /^((en|dis)able|(tru|fals)e|(turn)?o(n|ff)|[01])$/i;
nexus.rowner = true;
