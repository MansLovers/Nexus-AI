...
const user = ["592060314", "1894681713"];
const screet = ["fETGDoVAx5KsD3mtq6QfAFiS8R", "XLuPHkxCRNGTVJP4kDheJbvC5A"];
    ...
    let response = await axios.get(
      "https://api.sightengine.com/1.0/check.json",
      {
        params: {
          url: url,
          models: "nudity-2.0",
          api_user: await `${user.getRandom()}`,
          api_secret: await `${screet.getRandom()}`
        }
      }
    );
