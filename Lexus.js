 ...
 */
const { proto, getAggregateVotesInPollMessage } = (
...
export async function nexusfile(chatUpdate) {
  ...
  try {
    ...
    try {
      ...
      if (user) {
        ...
        if (!("karakter" in user)) user.karakter = "";
      ...
      if (chat) {
        ...
        if (!("antiSpam" in chat)) chat.antiSpaml = true;
    ...
    const isROwner = [
      ...
      .includes(m.sender);
    const isVip = isROwner || global.db.data.users[m.sender].vip;
    ...
    for (let name in global.nexus) {
      let nexus = global.nexus[name];
      if (!nexus) continue;
      if (nexus.disabled) continue;
      const __filename = join(___dirname, name);
      if (typeof nexus.all === "function") {
        try {
          await nexus.all.call(this, m, {
            chatUpdate,
...
`.trim(),
      ...
      if (!opts["restrict"])
        if (nexus.tags && nexus.tags.includes("admin")) {
          // global.dfail('restrict', m, this)
          continue;
        }
      const str2Regex = (str) => str.replace(/[|\\{}()[\]^$+*?.]/g, "\\$&");
      let _prefix = nexus.customPrefix
        ? nexus.customPrefix
        : conn.prefix
        ? conn.prefix
        : global.prefix;
      ...
      if (typeof nexus.before === "function") {
        if (
          await nexus.before.call(this, m, {
            match,
            conn: this,
            participants,
            groupMetadata,
      ...
      if (typeof nexus !== "function") continue;
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        let fail = nexus.fail || global.dfail; // When failed
        let isAccept =
          nexus.command instanceof RegExp // RegExp Mode?
            ? nexus.command.test(command)
            : Array.isArray(nexus.command) // Array?
            ? nexus.command.some((cmd) =>
                cmd instanceof RegExp // RegExp in Array?
                  ? cmd.test(command)
                  : cmd === command
              )
            : typeof nexus.command === "string" // String?
            ? nexus.command === command
            : false;
        ...
        if (
          ...
          if (
            ...
            return; // Except this
          if (name != "own-unbanuser.js" && user?.banned) return;
        }
        if (nexus.rowner && nexus.owner && !(isROwner || isOwner)) {
          // Both Owner
          fail("owner", m, this);
          continue;
        }
        if (nexus.rowner && !isROwner) {
          // Real Owner
          fail("rowner", m, this);
          continue;
        }
        if (nexus.owner && !isOwner) {
          // Number Owner
          fail("owner", m, this);
          continue;
        }
        if (nexus.mods && !isMods) {
          // Moderator
          fail("mods", m, this);
          continue;
        }
        ...
        if (nexus.premium && !isPrems) {
          // Premium
          fail("premium", m, this);
          continue;
        }
        if (nexus.group && !m.isGroup) {
          // Group Only
          fail("group", m, this);
          continue;
        } else if (nexus.botAdmin && !isBotAdmin) {
          // You Admin
          fail("botAdmin", m, this);
          continue;
        } else if (nexus.admin && !isAdmin) {
          // User Admin
          fail("admin", m, this);
          continue;
        }
        if (nexus.private && m.isGroup) {
          // Private Chat Only
          fail("private", m, this);
          continue;
        }
        if (nexus.register == true && _user.registered == false) {
          // Butuh daftar?
          fail("unreg", m, this);
          continue;
        }
        m.isCommand = true;
        let xp = "exp" in nexus ? parseInt(nexus.exp) : 23; // XP Earning per command
        if (xp > 200) m.reply("Ngecit -_-"); // Hehehe
        else m.exp += xp;
        if (
          !isPrems &&
          nexus.limit &&
          global.db.data.users[m.sender].limit < nexus.limit * 7
        ) {
          this.reply(
            m.chat,
            `Limit kamu sidah mencapai batas\n*segera beli limit atau premium ke owner*`,
            m
          );
          continue; // Limit habis
        }
        if (nexus.level > _user.level) {
          this.reply(
            m.chat,
            `Diperlukan Level ${nexus.level} Untuk Menggunakan Perintah Ini\n*Level Kamu:* ${_user.level}`,
            m
          );
        ...
        try {
          await nexus.call(this, m, extra);
          if (!isPrems) m.limit = m.limit || nexus.limit || false;
        } catch (e) {
          // Error occured
          m.error = e;
          console.error(e);
...
`.trim(),
          ...
          if (typeof nexus.after === "function") {
            try {
              await nexus.after.call(this, m, extra);
            } catch (e) {
              console.error(e);
            }
          }
        ...
        for (let user of participants) {
          ...
          try {
            ...
            text =
              action === "add"
                ...
                : (chat.sBye || this.bye || conn.bye || "Bye, @user!")
                    ...
                    .replace(
                      "@desc",
                      (await groupMetadata.desc?.toString()) || "unknown"
                    );
            ...
            const canWel = await new canvafy.WelcomeLeave()
              ...
              .setBackground(
                "image",
                "https://telegra.ph/file/39ef0462ab2a3cc5ebfcc.jpg"
              )
              .setTitle("Welcome")
            ...
            if (_wel.gcImg) {
              this.sendMessage(id, {
                ...
                contextInfo: {
                  externalAdReply: {
                    title:
                      action === "add" ? "W E L C O M E" : " G O O D B Y E",
                    body: "",
                    thumbnail: xnxx,
                    sourceUrl: sgc,
                    mediaType: 1,
              ...
              this.sendMessage(id, {
                ...
                contextInfo: {
                  externalAdReply: {
                    ...
                    mediaType: 1,
                    mediaUrl:
                      "https://telegra.ph/file/74abb87ac6082571db546.jpg",
                    renderLargerThumbnail: true,
...
global.dfail = (type, m, conn) => {
  const userTag = `👋 Halo Kak *@${m.sender.split("@")[0]}*, `;
  let msg = {
    rowner: `[ ! ] Fitur Ini Khusus Developers`,
    owner: `[ ! ] Fitur Ini Khusus Developers`,
    mods: `[ ! ] Fitur Ini Khusus Moderator`,
    premium: `[ ! ] Fitur Ini Khusus Premium`,
    vip: `[ ! ] Fitur Ini Khusus vip`,
    group: `[ ! ] Fitur Ini Khusus Di Dalam Group`,
    private: `[ ! ] Fitur Ini Khusus Di Private Chat`,
    admin: `[ ! ] Fitur Ini Khusus Admin Group`,
    botAdmin: `[ ! ] Hanya Bisa Ketika Bot Mejadi Admin`,
    unreg: `[ ! ] Silahkan Daftar Terlebih Dahulu [ ! ]\n\n\`Option\`\n1. .Daftar nama.umur\n2. .Daftar2\n3. .Regnom 628`,
    rpg: `[ ! ] Fitur Ini Khusus Yang Join RPG, Silahkan Ketik .joinrpg`,
  }[type];
...
watchFile(file, async () => {
  unwatchFile(file);
  console.log(chalk.redBright("Update 'Lexus.js'"));
  if (global.reloadnexusfile) console.log(await global.reloadnexusfile());
});
